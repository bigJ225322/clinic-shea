...
future-the owner read to know exactly what to do at each step. If conversation context
...
**built, live in the Maps tab, and polished to the owner's bar.** They are the canonical
...
### Build recipe when the owner says "build &lt;treatment&gt;"
1. **Get the appointment list from the owner first.** He's done these cases — his
...
2. **Verify every clinical claim against a TRUSTED source**, in priority order:
   the owner's firsthand experience → **Swade** (`/tmp/swade.txt`, greppable; re-extract
...
5. **Verify, then deploy:** `npx esbuild src/App.jsx --loader:.jsx=jsx --outfile=/dev/null`
   ...
   after the JSON-edit page reload) → deploy (`git push origin main`) → the owner reviews the
...
- **Don't invent clinical procedure.** CD had a fabricated "clinical remount +
  selective grinding at delivery" step — not in Swade, and the owner's buddy confirmed
  ...
  **stop and have the owner confirm with a peer who did the case** before writing it.
...
`~/Documents/Dentistry Files/ALL CD FILES/`.
...
**Explicitly disqualified — AI-GENERATED, NEVER TRUST (confirmed by the owner):**
The "CD Comprehensive Guide" family was **generated by AI, not authored by UIC
faculty.** It reads authoritative and some files even carry a "UIC" suffix, but
the clinical claims are unverified and HAVE introduced real errors into the
pathway (e.g. it omitted the lab-Rx sign-off / denture-consent gate between the
posterior try-in and lab processing, and muddied who festoons). Check this list
BEFORE pointing any read/research at a file in the CD folder:
- `"Comprehensive CD Guide".docx` — AI-generated compiled guide
- `CD Comprehensive Guide.html` — AI-generated compiled guide
- `CD Comprehensive Appointment Guide (UIC).pdf` — AI-derived; the "UIC" suffix
  is MISLEADING — it is NOT a faculty document
- `CD Comprehensive Guide — Deep Dives (UIC).pdf` — same
- Any `.docx` content from the folder

**Trusted-source criteria (priority order):**
1. the owner's own clinical experience / direct confirmation (he has done a CD).
2. **Swade** (`clinic (swade).pdf`) — the manual.
3. Named, dated, faculty-attributed UIC lecture decks / lab worksheets /
   grading sheets.
An AI-generated compilation is **never** authoritative, no matter how polished
it reads or what its filename claims. A "UIC" in the filename is not proof of
provenance. When in doubt, ask the owner rather than trust a compiled guide.
...
6. **Commit** with a message that:
   ...
   - Calls out any decision points where the PDFs were ambiguous or
     contradictory (so the owner can review)
...
Required UIC PDFs (`~/Desktop/HY Folders/Dentistry Files/01 Clinical Reference/Removable Partial Dentures/ALL RPD FILES/`):
...
Required UIC PDFs (`~/Desktop/HY Folders/Dentistry Files/01 Clinical Reference/Fixed Prosthodontics/`):
...
Dentistry Files root (`~/Desktop/HY Folders/Dentistry Files/`).
A path of `01CR/` means `01 Clinical Reference/`.

If a pathway's row says "**Gap**: ...", we don't have enough UIC source to
write it confidently yet — flag for the owner to share more content, or skip
...
| `cd-implant-supported-lower` (2-implant Mn OD) | **Gap**: same as above. `01CR/Implantology/IMPLANT TBL LECTURE.pptx` may have attachment coverage. Han lecture slides 49+ touch on IOD context. Likely need additional UIC IOD-specific content from the owner. |
...
| `dir-class2` | `01CR/Operative and Restorative/From All folder/HUDDLE for Class II composite.docx`; `Class II Amalgam #3 MO.pdf`; `Class II PE Instructions 2024.pdf`; `[author]_Slot Prep Tips Part 1_2024.pdf` + `Part 2_2024.pdf` (these are by the owner!) |
...
| `dir-direct-veneer` | `01CR/Operative and Restorative/Direct Veneer Pics/*.png` (10+ technique screenshots — these are the owner's reference images for direct veneer technique); Swade chunks for note template |
...
coverage already), or (b) flagging for the owner to share more Pedo lecture
...
Pathways flagged with **Gap** above (need additional content from the owner or
...
be addressed later as the owner shares more content or as we accept that some
...
## Resume points for future Claude (or future the owner)
...
## Scope clarifications from the owner (2026-05-27) — READ BEFORE BUILDING ANY OF THESE FAMILIES
...
the owner's words: "we are only loosely involved with implants — i actually
...
the owner's words: "we really just extract teeth. There's some students that
...
the owner's words: "I'm hoping you keep a good scope of Perio too. We have to
...
the owner's words: "Acute perio abscess is literally just an outcome of an
...
the owner's words: "I don't think we do veneers at all. Don't do any veneer
stuff."
...
- ❌ `dir-direct-veneer` — chairside direct composite veneer (also a
  veneer; the owner said "don't do any veneer stuff")
- ❌ `repair-veneer-debond`

### Zirconia repairs — shelved

the owner's words: "Idk if we even do zirconia repairs. Shelf that one for
now."
...
the owner's words: "Pedo stuff is also not very multi step, and there are zero

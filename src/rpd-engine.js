...
const RPD_RATIONALE = {
  major: {
    "A-P Strap":            "Rigid major connector of choice for Maxillary Kennedy Class I and II (Lecture 6 p. 8). Two transverse straps (8 mm anterior, 6-8 mm posterior) connected laterally provide cross-arch rigidity essential for distal extension support.",
    ...
    "Single Palatal Strap": "Rigid major connector of choice for short-span Maxillary Class III (Lecture 6 p. 8). Single transverse strap, 8 mm minimum width, positioned across rugae with 6 mm clearance from gingival margins anteriorly and 6 mm anterior to vibrating line posteriorly.",
    "Full Palatal Plate":   "Connector of choice when few abutments remain (≤4 abutments, typically distributed) (Lecture 6 p. 11). Maximum stability + retention via broad palatal coverage; posterior border AT vibrating line. Rigid plate; not for severe gag reflex patients.",
    "U-Shaped Connector":   "Used due to large inoperable maxillary tori or severe sensitive gag reflex that cannot tolerate palatal coverage (Lecture 6 p. 12). Least rigid maxillary major connector — last-resort choice; avoid for Class I/II distal extensions if at all possible.",
    "Lingual Bar":          "Major connector of choice when sufficient lingual sulcus space exists (≥8 mm from gingival margin to floor of mouth) (Lecture 6 p. 13). Half-pear cross-section, 4 mm vertical height, superior border ≥3 mm below gingival margins. Minimal tissue coverage, easiest for patient to adapt to.",
    "Lingual Plate":        "Connector of choice when lingual sulcus depth is insufficient (<8 mm) (Lecture 6 p. 14). Extends from same superior position as Lingual Bar onto cingula of anterior teeth; finishes on tooth surface (not gingival margin). MUST be paired with rests at terminal abutments to prevent ortho force on anteriors.",
    "Lingual Plate (Tori)": "Used due to presence of mandibular tori obstructing the Lingual Bar inferior border (Lecture 6 p. 15). Plate covers tori and routes superior to them; surgical removal of tori is the preferred alternative if patient is amenable.",
    "Sublingual Bar":       "Alternative to Lingual Bar when high lingual frenum / shallow lingual sulcus prevents standard Lingual Bar AND Lingual Plate is contraindicated (healthy mobile anteriors, perio concerns with lingual plate contact). Bar sits in sublingual space below standard Lingual Bar position. Uncommon at UIC predoctoral.",
  },
  clasp: {
    "Akers":                "Cast circumferential clasp of choice for tooth-supported abutments (Kennedy III/IV) (Retainers PDF p. 14). Engages 0.01\" buccal undercut on mesial half of tooth; rigid until terminal third where it flexes into undercut. Reciprocation by opposite-arch plating or cast lingual arm. NOT for distal extension (causes Class I torquing).",
    "RPI":                  "Stress-releasing clasp assembly of choice for distal-extension abutments (Kennedy I/II) (Retainers PDF p. 16, 31, 35). Rest mesial + Proximal plate distal (UIC house spec: covers 1/2 to 2/3 of the occluso-gingival height of the distal proximal surface — Kratochvil-vs-Krol compromise, Retainers p. 31) + I-bar engaging 0.01\" mid-buccal undercut at gingival 1/3. I-bar geometry: 3 mm horizontal arm + 1.5 mm vertical engagement (Retainers p. 32). Vertical-projection I-bar rotates AWAY from abutment under occlusal load → stress-release.",
    "Combination":          "Stress-releasing clasp assembly when RPI is contraindicated (vestibular depth <5 mm, undercut location mid-mesial/MB/DB) (Retainers PDF p. 20). Cast rest + 18ga wrought-wire C-clasp arm engaging 0.02\" undercut; wrought wire flexes more than cast metal under stress.",
    "I-bar (esthetic)":     "Clasp of choice for the esthetic zone (canine/premolar) when display is high (Retainers PDF p. 19). I-bar approach is invisible below gingival line; rest seat is cingulum (canine) or occlusal (premolar). Requires ≥5 mm vestibular depth and a buccal undercut.",
    ...
    "Embrasure":            "Clasp of choice for a quadrant with no edentulous space (Class III mod 0 where retention is needed on a quadrant where no teeth are missing) (Retainers PDF p. 24). Two opposing C-clasps cradle adjacent teeth at the embrasure; each engages a 0.01\" undercut. Requires a prepped occlusal rest seat at the embrasure between the two teeth.",
    "Ring":                 "Cast retainer for a tilted molar abutment where multiple undercut surfaces require encirclement (Retainers PDF p. 26). 360° wraparound with auxiliary occlusal rest; rare in modern practice.",
...
function rpdSelectMajorConnector(caseInput, kennedy) {
  ...
  if (kennedy.class === "III") {
    ...
    if (!isBilateral) {
      ...
      if (totalMissing <= 3) {
        // UIC's curriculum has no unilateral RPD design for short tooth-
        ...
        // first choice; if the patient declines fixed, UIC's fallback is
        ...
        return {
          ...
          alternativeRationale: "Primary alternative if the patient declines bridging: individual implant crowns at each missing-tooth site. If the patient declines BOTH FPD and implants (cost, surgical contraindication), UIC indicates an Interim Partial Denture (IPD) with non-metal (e.g., Valplast/Duraflex thermoplastic) clasps — explicitly indicated for Class III / short-span IV cases per the UIC IPD lecture. Unilateral cantilever (Nesbit) RPDs are banned at UIC due to aspiration risk; definitive cast-metal RPD is not appropriate for this configuration.",
        };
      }
    ...
    if (sides.size < 2) {
      ...
      if (pair) {
        ...
        if (akers != null && !claimed.has(akers)) {
          ...
          notDesignable = {
            reason: "Class III configuration with all span-derived abutments on one side AND no viable contralateral retention possible (no acceptable posterior tooth on the missing side). RPD design would be unilateral (Nesbit), which is prohibited (McCracken p. 69 + UIC Final RPD Huddle 2 Q2 — aspiration risk). Recommend fixed prosthesis (FPD or implant) for this configuration.",
          };
        }
      }
    }
 ...
 * Per UIC Lecture 4A p. 9, four independent criteria; ANY one triggers a
 ...
 * UIC's no-clasp rule: max anteriors bounding tooth-supported modification
 ...
 * an alternative. UIC Summer 2023 IPD lecture.
 */
...
function pickGuidePlane({ tooth, isDE, sideToward, claspType }) {
  ...
  const rationale = length.startsWith("long")
    ? RPD_RATIONALE.guidePlane.longParallel
    : RPD_RATIONALE.guidePlane.standard;
 ...
 *   - Combination: always engages mesio-buccal (UIC convention, Retainers
...
/**
 * Determine if the case warrants a Ring clasp alternative note. UIC option
    ...
    return {
      ...
      claspRationale: RPD_RATIONALE.clasp["Reverse Akers"]
        ...
        + (isBicuspidOrCanine ? "" :
            ...
            " — UIC Huddle 6 Q10 carve-out: lingually inclined terminal molar permits Reverse Akers (or Ring clasp). The tilt makes lingual surface the practical engagement location." :
            " — NOTE: UIC restricts Reverse Akers to bicuspids and cuspids (or tilted terminal molars per Huddle 6 Q10); this tooth is outside that indication scope. Consider Ring clasp instead for a tilted molar with distal undercut."),
      ...
      claspAlternativeRationale: isBicuspidOrCanine
        ...
        : isTiltedTerminalMolar
        ? "Ring clasp is the alternative for tilted terminal molars with lingual undercut: ~360° encirclement with primary mesial occlusal rest + distal auxiliary rest + supportive strut. Both Reverse Akers AND Ring are UIC-acceptable for this scenario (Huddle 6 Q10)."
        : "Ring clasp is UIC's typical choice for a tilted molar with a distal undercut: ~360° encirclement with primary mesial occlusal rest + distal auxiliary rest + supportive strut. Reverse Akers on a non-tilted molar is off-label per the Retainers lecture scope.",
    };
  }
...
function rpdDesignAbutment({ tooth, span, caseInput, kennedy, attrs, vestibularLow }) {
  ...
  // Interim branch — UIC Summer 2023 IPD lecture (WW C-clasp default)
  ...
  // Ring clasp alternative for tilted molars (UIC Retainers PDF — surfaced
...
// are dimensioned per UIC textbook defaults for the role.
...
function rpdDesignEmbrasureAbutment({ tooth, pairedWith, caseInput, kennedy, attrs }) {
  ...
  if (designIntent === "interim") {
    return {
      ...
      claspRationale: RPD_RATIONALE.clasp["WW C-clasp"] +
        ` Paired with a WW C-clasp on ${partner} for contralateral retention — avoids unilateral (Nesbit) design (UIC PROHIBITS, Final RPD Huddle 2 answer key Q2, aspiration risk).`,
      ...
      claspAlternativeRationale: "Ball clasp is the UIC IPD alternative when no usable circumferential undercut is present — engages between the two teeth at the buccal embrasure without surveying a single-tooth undercut.",
    };
  }
  ...
  const restDim = isMolar
    ? "spoon-shaped depression 1.5 mm deep × 2.5 mm M-D × 2.5 mm B-L"
    : "spoon-shaped depression 1.0 mm deep × 2.0 mm M-D × 2.0 mm B-L";
  // UIC bur convention (Lab 5 p. 8) — molar rests use #8 round outline +
  // #6 axial; premolar rests use #6 round outline + #4 axial. Same burs
  // used for the shared embrasure rest.
  const burText = isMolar
    ? "#8 round (outline) / #6 round (axial inclination)"
    : "#6 round (outline) / #4 round (axial inclination)";
  ...
  return {
    ...
    claspRationale: RPD_RATIONALE.clasp["Embrasure"] +
      ` This Embrasure clasp provides PRIMARY cross-arch retention paired with ${partner}, required to satisfy the bilateral direct-retention invariant. Without it the design would be unilateral (Nesbit) — UIC PROHIBITS Nesbit RPDs (Final RPD Huddle 2 answer key Q2, aspiration risk).`,
...
function rpdDesignContralateralAkers({ tooth, caseInput, kennedy, attrs }) {
  ...
  return {
    ...
    claspRationale: RPD_RATIONALE.clasp["Akers"] +
      " Engine added this Akers as contralateral retention because no viable Embrasure pair exists on this side. Without it, the design would be unilateral (Nesbit), which UIC PROHIBITS (Final RPD Huddle 2 answer key Q2, aspiration risk).",
...
function rpdPlaceIndirectRetainers(caseInput, kennedy) {
  ...
  if (kennedy.class === "IV") {
    ...
    for (const t of bounding) {
      ...
      // Per UIC Final Huddle Week 6 Q8 + Design Case II:
      ...
      if (RPD_CANINES.has(t)) {
        ...
        // UIC permits distal ball rest serving dual direct+indirect role.
      ...
      out.push({
        ...
        rationale: RPD_RATIONALE.rest.classIVAnterior +
          (isMaxCentral
            ? " Per UIC Huddle 6 Q8, a distal ball rest on a maxillary central incisor bounding the anterior span can serve simultaneously as direct retainer AND indirect retainer."
            : ""),
...
function rpdCheckRedFlags(caseInput, kennedy, abutmentDesigns) {
  ...
  if (kennedy.class === "III") {
    ...
    if (rightAbuts.length === 0 || leftAbuts.length === 0) {
      ...
      const ctrlRetainers = (abutmentDesigns || [])
        .filter(a => a.role === "embrasure-retention" || a.role === "contralateral-akers");
      const ctrlSummary = ctrlRetainers.length === 0
        ? "Engine could not find acceptable contralateral retention (no viable posterior tooth on the opposite side) — RPD is contraindicated for this configuration as designed."
        : ctrlRetainers[0].role === "embrasure-retention"
          ? `Engine added Embrasure clasp pair on ${ctrlRetainers.map(a => `#${a.tooth}`).join(" + ")} for the required contralateral retention.`
          : `Engine added single Akers on #${ctrlRetainers[0].tooth} for contralateral retention (no viable embrasure pair on that side).`;
      flags.push({
        severity: "info",
  ...
  // NMCD — Fueki 2016 two-tier split + UIC contraindication list (Fall RPD
  // 22 Non-metal clasps lecture pp. 2-6).
  if (pf.metalAllergy) {
    flags.push({
      severity: "warning",
      type: "nmcd",
  ...
  if (pf.opposingArch === "complete_denture"
    ...
    flags.push({
      ...
      message: "Opposing arch is a complete denture. UIC requires BILATERAL BALANCED ARTICULATION including protrusive and lateral excursive movements for this RPD (Combination Syndrome prevention; Lecture 3 p. 4). At wax-rim try-in, set teeth for simultaneous bilateral contacts in MI, protrusion, and lateral excursions. Use the Denar facebow + bite registration to mount casts. Selective pressure or altered cast impression technique is also required.",
    });
  }

  // Kennedy Class I distal extension — UIC Occlusion lecture p. 4 splits
  // this rule BY ARCH:
  ...
  // The earlier Lecture 3 p. 4 "balanced is also strived in Class I" is
  ...
  if ((pf.opposingArch === "natural" || pf.opposingArch === "existing_partial" || !pf.opposingArch)
    ...
    flags.push({
      ...
      message: "Occlusion scheme (Lecture 3 p. 4): GROUP FUNCTION on the working side. Denture teeth on the NON-WORKING (balancing) side MUST have NO contact during excursion — UIC's explicit prohibition. Excursive movement should happen on natural teeth whenever possible; only balance forces on denture teeth if natural-tooth guidance is unavailable. Canine guidance is acceptable if patient's natural dentition exhibits it. At delivery, verify with AccuFilm II: working-side contacts at premolar + molar denture teeth, balancing-side fully clear during lateral excursions. Adjust premature contacts on the DENTURE TEETH, never on natural teeth.",
    });
  }
...
function rpdGenerateLabScript({ arch, framework, majorConnector, abutmentDesigns, indirectRetainers, baseDesigns, designIntent, axiumCode, kennedy }) {
  ...
  // Saddle base sulcus engagement (Huddle 6 p. 1 Q11 — verbatim UIC rule).
  ...
  if (hasDistalExtension) {
    lines.push("");
    if (arch === "mandibular") {
      lines.push("Distal extension saddle acrylic: must engage the sulcus IN ITS ENTIRETY — BOTH buccally AND lingually (Huddle 6 Q11 — UIC verbatim rule). Maximum tissue contact distributes occlusal load across the residual ridge and resists framework rotation under load.");
    } else {
...
const RPD_AXIUM_STEPS = [
  ...
  },
  {
    ...
    design: "Wax try-in is the LAST chance to adjust tooth position, shade, or VDO before processing. Lab cannot reverse acrylic processing. Use UIC tray sheet to document patient approval with signature.",
  },

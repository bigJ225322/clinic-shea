...
- **Lab 6 (process):** compression-molding processing + boil-out + lab remount + finish/polish around the framework — Sw lab step #7 + the RPD processing convention (the CD-side Ivocap conflict doesn't arise here). ✅ **✂ FIXED 2026-06-12 (the owner caught it):** the detail wrote invest/boil-out/pack/deflask/finish as IMPERATIVES to the student — the same leaked lab-as-student framing the CD pathway's L5 was purged of on 2026-05-29. **Sw lab step #7 is one line: "send RPD to lab for processing"** — the student's touches are the Axium lab-script handoff and the pickup. Tile retitled "Send to lab for processing"; processing steps now attributed to the lab in descriptive voice (matches the bridge framework/porcelain lab-step voice). Technique content unchanged (flask/boil-out/pack/heat-process/remount/finish — standard compression-molding sequence, kept as description of what the lab does).
...
## Source-or-cut autonomous pass — 2026-06-13
**✂ CUT (unsourced specifics):** Lab 2 (survey + design) detail said path of insertion "roughly perpendicular to the occlusal plane **(within ~10°), the rod contacting the occlusal ½–2/3 of the proximal surfaces**." The RPD Lab-4 design doc is image-only (43 lines extractable, none of these numbers). The only "perpendicular to the occlusal plane" in the RPD corpus is RPD Fall 2025 Lab 3 — about HOLDING THE HANDPIECE for guide-plane prep, NOT the survey tilt. The "~10°" tolerance and "½–2/3 rod contact" are unsourced → cut. Kept the qualitative "minimal tilt, roughly perpendicular to the occlusal plane." (The ledger had self-flagged these ⏸ image-only; now resolved by cutting.) The 0.01/0.02/0.03 gauge SET stays — sourced to the UIC Preliminary Design form (real Ney gauge increments).

## RPD chapter re-audit — 2026-06-13 (the owner: "round out #12")
Audited the live `rpdc-*` Maps chapters (guides-data) against this ledger + the trove lab PDFs. **Key finding: a sync gap** — four corrections this ledger recorded were applied to the pathway-level content (App.jsx) but NOT to the `rpdc-*` chapters the Maps pathway renders, so stale/cut values were still live. Propagated all four (commit on 2026-06-13):
- **rpdc-facebow** condylar `25-30°` → **25°** — `[3] RPD Lab_ Facebow & Mounting Casts` verbatim: "Condylar Inclination: 25, Incisal Pin at Zero". (Bennett 15° / pin 0 already correct.)
- **rpdc-records** "cure the intaglio for **20 sec**" → "**briefly**" — the 20-sec figure is unverifiable (matches this ledger's earlier cut).
- **rpdc-design** survey rod "perpendicular to occlusal plane **(within ~10°)** … occlusal **½–2/3** of the proximal surfaces" → cut both specifics (kept the qualitative claim). Same cut already made to the pathway keyDecision; the chapter had retained it.
- **rpdc-finalimp** custom-tray cure "**~2 min, then the intaglio ~1 min**" → "**1 min / rest / 1 min / intaglio 1 min**" (Kim-Tray formula) — same fix the pathway labStep got.
